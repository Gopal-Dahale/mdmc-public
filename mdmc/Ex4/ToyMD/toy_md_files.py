...
def write_pdb_frame(file, step, box, coords, atomnm, resnm, resnr, elem,
    ...
    for i in range(N):
        file.write(
            "ATOM   %4d %4s%4s  %4d    %8.3f%8.3f%8.3f%6.2f%6.2f          %2s\n"
            % (i + 1, atomnm[i], resnm[i], resnr[i] + 1, 10 * coords[i][0],
               10 * coords[i][1], 10 * coords[i][2], 1.0, 0.0, elem[i]))
    file.write("TER\n")
    if (conect):
        for c in conect:
