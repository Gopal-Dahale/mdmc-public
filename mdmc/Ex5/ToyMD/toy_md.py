...
def parseArguments():
    ...
    parser.add_argument("-c",
                        "--coordinates",
                        dest="coordinates",
                        help="Coordinate pdb file for reading",
                        type=str,
                        default=None)
    parser.add_argument("-o",
                        "--trajectory",
                        dest="trajectory",
                        help="Output pdb file for writing",
                        type=str,
                        default="traj.pdb")
    parser.add_argument("-w",
                        "--outcoords",
                        dest="outcoords",
                        help="Coordinate pdb file for writing and restarting",
                        type=str,
                        default=None)
    parser.add_argument("-p",
                        "--parameters",
                        dest="parameters",
                        help="Parameter file for reading",
                        type=str,
                        default=None)
...
if __name__ == '__main__':
    ...
    [box, coords, atomnm, resnm, resnr, elem, elem_n,
     conect] = read_pdb(args.coordinates)
    ...
    #Initial Force
    [epot, forces] = calculate_forces(box, coords, elem, elem_n, conect,
                                      exclude, ff)
    ...
    kB = 0.00831415
    T = float(md_params["temperature"])
    
    # Calculate scale factors for each particle based on mass and desired temperature
    # From equipartition theorem: 1/2 m<v^2> = 3/2 kT
    scale_factors = np.sqrt(kB * T / masses)
    
    # Scale velocities for each particle (broadcasting the scale factors)
    ...
    for step in range(int(md_params["number-of-steps"])):
        ...
        coords += velocities * time_step + 0.5 * (
            forces / masses[:, np.newaxis]) * time_step**2
        ...
        # Update forces
        # f(t+dt)= f(r(t+dt))
        [epotential, forces] = calculate_forces(box, coords, elem, elem_n,
                                                conect, exclude, ff)
        ...
        # Print some stuff
        print(
            "Step: %5d Epot %10.3f Ekin %10.3f Etot %10.3f T %7.2f lambda %.2f"
            % (step, epotential, ekinetic, epotential + ekinetic, T, lambda_T))

        if (step % int(md_params["output-frequency"]) == 0):
            write_pdb_frame(outputfile, step, box, coords, atomnm, resnm,
                            resnr, elem, None)
            write_log(logfile, step, epotential, ekinetic,
                      epotential + ekinetic, T, lambda_T)
    ...
    if (args.outcoords):
        ...
        write_pdb_frame(outputfile, step, box, coords, atomnm, resnm, resnr,
                        elem, conect_orig)
