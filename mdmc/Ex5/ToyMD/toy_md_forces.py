...
def distance_pbc(box, x1, x2):
    ...
    dx = x1 - x2 # Raw difference: [dx, dy, dz]
    
    # Apply minimum image convention
    # Adjust dx if |dx_k| > L_k/2 by subtracting L_k * round(dx_k / L_k)
...
@njit
def nonbonded_forces(box, coords, elem, exclude, force, sigma, epsilon,
                     charge):
    energy = 0
    N = len(coords)
...
def calculate_forces(box, coords, elem, elem_n, conect, exclude, ff):
    ...
    [Vnonbond,
     force] = nonbonded_forces(box, coords, elem_n, exclude, force,
